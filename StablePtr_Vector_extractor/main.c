...
// Wrapper
C_DoubleVector fromStablePtr_vector_hs_double_wrapper(HsStablePtr stablePtr_vector_double)
{   // C_DoubleVector vec_double; // Without inizialization (faster but harder to debug).
    ...
    fromStablePtr_vector_double_hs(stablePtr_vector_double,  &(vec_double.c_doubleVectorSize), &(vec_double.c_doubleVectorPtr));
    return vec_double;
}

// Wrapper
C_IntVector fromStablePtr_vector_hs_int_wrapper(HsStablePtr stablePtr_vector_int)
{   // C_IntVector vec_int; // Without inizialization (faster but harder to debug).
    ...
    fromStablePtr_vector_int_hs(stablePtr_vector_int,  &(vec_int.c_intVectorSize), &(vec_int.c_intVectorPtr));
    return vec_int;
}
...
{   if (argc < 2) {printf("Usage: %s <intNumber>\n", argv[0]);return 2;}
    ...
    HsStablePtr stablePtr_vector_double = somethingToVec_double_hs(arg_int);

    C_DoubleVector vec_double = fromStablePtr_vector_hs_double_wrapper(stablePtr_vector_double);
    printf("result:     "); doubleVecPtr_show(vec_double);              printf("\n");

    vector_double_free_hs(stablePtr_vector_double); // From now on the internal array of vec_double is not available!




    HsStablePtr stablePtr_vector_int = somethingToVec_int_hs(arg_int);

    C_IntVector vec_int = fromStablePtr_vector_hs_int_wrapper(stablePtr_vector_int);
    printf("result:     "); intVecPtr_show(vec_int);              printf("\n");

    vector_int_free_hs(stablePtr_vector_int); // From now on the internal array of vec_int is not available!
